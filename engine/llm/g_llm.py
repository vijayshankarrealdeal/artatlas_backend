...
from engine.models.artworks_model import ArtworkData, AudioQuery, LLMInputPayload # Assuming these are defined elsewhere
...
def llm_generate_artwork_metadata(payload: LLMInputPayload) -> ArtworkData:
    ...
    query_text = processed_payload.get("query") # This is the user query/instruction for the LLM
    ...
    try:
        ...
        logger.error(f"Failed to open image from {image_url}. It might be corrupted or not a valid image format. Error: {e}")
        logger.exception("UnidentifiedImageError details:")
        raise
    except Exception as e:
        logger.error(f"An unexpected error occurred during image fetching or opening from {image_url}. Error: {e}")
    ...
    contents = [pil_image, query_text] # Assuming query_text is the prompt/instruction for the LLM
    ...
    llm_output = None
    try:
        logger.info(f"Sending request to Gemini model: {model_name} for artwork metadata generation.")
        ...
        logger.debug(f"Raw LLM output text (first 500 chars): {llm_output.text[:500] if llm_output and hasattr(llm_output, 'text') else 'No text output'}")
    except Exception as e:
    ...
    try:
        ...
        logger.debug(f"LLM response text that failed parsing: {llm_output.text if llm_output else 'No LLM output'}")
        logger.exception("JSONDecodeError details:")
        raise
    except Exception as e:
        logger.error(f"An unexpected error occurred during LLM response parsing. Error: {e}")
    ...
    original_data_payload = processed_payload.get("payload", {}) # Assuming "payload" key holds the original data dict
    for k, v in original_data_payload.items():
        ...
        elif k not in result_dict or result_dict[k] is None: # Prioritize LLM results unless they are None
            result_dict[k] = v
            logger.debug(f"Merged key '{k}' from original payload.")
        else:
            logger.debug(f"Key '{k}' already present in LLM result or LLM result is not None, not overwriting from original payload.")
...
def llm_generate_audio_to_text(audio_bytes: bytes, artwork_json: dict) -> str:
    ...
    logger.debug(f"Received audio bytes (length: {len(audio_bytes) if audio_bytes else 0}). Artwork JSON: {artwork_json}")
    ...
    generate_content_config = types.GenerateContentConfig(
        ...
        system_instruction=[
            types.Part.from_text(text="You are an AI assistant. Transcribe the audio query and then answer it. Your answer must be based *only* on the artwork information provided in the user's message context. If the query is unrelated to the artwork, state that you cannot answer. The response should be in JSON format matching the AudioQuery schema, with a 'response' field containing your textual answer."),
        ],
    )
    llm_output_text = None
    try:
        logger.info(f"Sending request to Gemini model: {model_name} for audio-to-text and query answering.")
        ...
        if not llm_output or not hasattr(llm_output, 'text') or not llm_output.text:
            logger.error("LLM output for audio processing is empty or does not contain text.")
            raise ValueError("LLM response for audio is empty or invalid.")

        llm_output_text = llm_output.text
        logger.debug(f"Raw LLM output text for audio: {llm_output_text}")

        # The function is expected to return the text (which is the JSON string in this case as per original code)
        # If it's meant to return the parsed AudioQuery object's 'response' field, that would be an additional step.
        # The original `return output.text` suggests returning the raw JSON string.
    except Exception as e:
